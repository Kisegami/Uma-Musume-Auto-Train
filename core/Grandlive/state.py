...
from core.Grandlive.skill_auto_purchase import (
    click_image_button,
    dismiss_skill_result_dialog,
    execute_skill_purchases,
    extract_skill_points,
)
from core.Grandlive.skill_recognizer import scan_all_skills_with_scroll, get_skills_api
from core.Grandlive.skill_purchase_optimizer import load_skill_config, create_purchase_plan, filter_affordable_skills
...
def check_skill_points(screenshot=None):
    ...
    if result > 0:
        from core.Grandlive.skill_auto_purchase import cache_skill_points
...
def _execute_auto_skill_purchase(
    ...
    When `force` is True, skip the skill-point-cap gate. `require_api` keeps
    API-only flows separate from whether their configured cap is enforced.
    """
    try:
        config = load_main_config()
    except Exception as e:
    ...
    skill_point_cap = (
        ...
        else skills_config.get("skill_point_cap", config.get("skill_point_cap", 9999))
    )
...
def check_and_purchase_skills_before_custom_race(screenshot=None):
    ...
    list without opening the skill screen first.
    """
    try:
        config = load_main_config()
    except Exception as e:
