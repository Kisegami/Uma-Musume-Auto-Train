...
from core.Trackblazer.state import check_skill_points_cap, check_current_year
from core.Trackblazer.ocr import extract_text
import os

# Helper function to get project root directory
def _get_project_root():
    """Get the project root directory (3 levels up from core/Trackblazer/)"""
...
def find_and_do_race(year_override=None):
    ...
    try:
        ...
        if skip_goal_check:
            ...
            from core.Trackblazer.state import check_goal_name
        ...
        if not best_race:
            log_info(f"No suitable race found")
            return False
...
def do_custom_race(year_override=None):
    ...
    try:
        ...
        if search_method == "template_matching":
            ...
            log_info(f"Custom Race - Template matching failed for '{custom_race}'")
            # Fall through to the common not-found/back-navigation handling.
...
def get_custom_race_selection(year_override=None):
    ...
    if isinstance(raw_value, dict):
        return {
            ...
            "use_hammer": bool(raw_value.get("use_hammer", False)),
        }

    if isinstance(raw_value, str) and raw_value.strip():
        return {"race": raw_value, "use_glowstick": False, "use_hammer": False}
    return None
