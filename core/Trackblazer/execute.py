...
def career_lobby(timeout=None):
    ...
    while True:
        ...
        cancel_matches = match_template(
            screenshot,
            "assets/buttons/cancel_lobby.png",
            confidence=0.8,
            region=get_template_region("assets/buttons/cancel_lobby.png"),
        )
        if cancel_matches:
            x, y, w, h = cancel_matches[0]
            center = (x + w//2, y + h//2)
        ...
        if close_matches:
            x, y, w, h = close_matches[0]
            center = (x + w//2, y + h//2)
            log_info(f"Close button appeared after lobby confirmation, clicking it.")
            tap(center[0], center[1])
            time.sleep(LOBBY_PRE_TURN_TAP_DELAY)
            continue
        ...
        item_state_for_training = normalize_item_state(raw_api_status, results_training) if _API_MODE and raw_api_status else None
        best_training = choose_best_training(
            results_training,
            training_config,
            current_stats,
            year=year,
            item_state=item_state_for_training,
        )
        relaxed_training_config = _build_relaxed_training_config(training_config)
        relaxed_training_candidate = choose_best_training(
            results_training,
            relaxed_training_config,
            current_stats,
            year=year,
            item_state=item_state_for_training,
        )
